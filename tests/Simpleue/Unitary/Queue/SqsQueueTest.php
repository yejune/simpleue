...
class SqsQueueTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testFailed()
    {
        ...
        $ReceipHandle = 'MyReceiptHandler';
        $job          = ['Body' => $data, 'ReceiptHandle' => $ReceipHandle];
        $this->sqsClientMock->expects($this->once())->method('sendMessage')->with(['QueueUrl' => 'queue-url-failed', 'MessageBody' => $this->sqsQueue->toString($data)]);
        $this->sqsClientMock->expects($this->once())->method('deleteMessage')->with(['QueueUrl' => 'queue-url-source', 'ReceiptHandle' => $ReceipHandle]);
    ...
    public function testError()
    {
        ...
        $ReceipHandle = 'MyReceiptHandler';
        $job          = ['Body' => $data, 'ReceiptHandle' => $ReceipHandle];
        $this->sqsClientMock->expects($this->once())->method('sendMessage')->with(['QueueUrl' => 'queue-url-error', 'MessageBody' => $this->sqsQueue->toString($data)]);
        $this->sqsClientMock->expects($this->once())->method('deleteMessage')->with(['QueueUrl' => 'queue-url-source', 'ReceiptHandle' => $ReceipHandle]);
